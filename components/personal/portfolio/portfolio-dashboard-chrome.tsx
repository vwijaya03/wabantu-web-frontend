  ...
  return (
    <div
      className={`max-w-full overflow-hidden rounded-2xl border border-neutral-200/80 bg-white shadow-[0_20px_60px_-24px_rgba(0,0,0,0.25)] ${
        compact || deck ? "text-[11px]" : "text-sm"
      }`}
    >
      <div
        className={`flex ${
          deck ? "min-h-0" : compact ? "min-h-[320px]" : "min-h-[420px]"
        }`}
      >
        <aside
          className={`shrink-0 border-r border-neutral-200/80 bg-neutral-50/80 ${
            deck ? "w-[160px]" : compact ? "hidden w-[180px] sm:block" : "hidden w-[220px] sm:block"
          }`}
        >
          ...
          <nav className={deck ? "space-y-1 p-2.5" : "space-y-0.5 p-2"}>
            {navItems.map((item) => {
